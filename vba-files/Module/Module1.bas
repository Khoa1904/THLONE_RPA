...
Sub p17_Macro_Main(pWeekOfYear As Integer, pWeekOfMonth As Integer, pHalfYear As Integer, pQuarter As String)
...
'    Dim currentDate As Date
    ...
    ProcessHomeplus wbBIFile, wbParkingFile
    
    ProcessLotte wbBIFile, wbParkingFile
    
End Sub

 

Function ProcessLotte(pWbBIFile As Workbook, wbParkingData As Workbook)
    'slide 38
    '1.Open “롯데마트_재고” file.
    Dim coupangFileName As String
    coupangFileName = COUPANG + "_" + DateAdd("d", -1, Date) + "_재고.xls"
    
    Dim wbLotte As Workbook
    Set wbLotte = Workbooks.Open(FOLDER_PATH_DOWN + coupangFileName)
...
Function ProcessHomeplus(pWbBIFile As Workbook, pWbParkingFile As Workbook)

    'slide 32
    '3. Activate ‘매출데이터’ Sheet
    
    Dim wsSalesData As Worksheet
    Set wsSalesData = pWbBIFile.Worksheets(SHEET_SALES_DATA)
    
    '4. Insert datas from ‘주차별데이터_MM월NN주차_내부정리용.xlsx” file
    Dim wsHomeplus As Worksheet
    Set wsHomeplus = pWbParkingFile.Worksheets(HOMEPLUS)
    wsHomeplus.AutoFilterMode = False
    
    
    Dim lastVisibleRow As Integer
    Dim lastVisibleCol As Integer
    ...
    dataCopy = wsHomeplus.Range("G3" + ":" + "M" + CStr(lastVisibleRow))
    dataCopy.Copy
    ...
    wbMaterial = Workbooks.Open(FOLDER_PATH_TEMPLATE + MATERIAL_FOLDER + "\" + MATERIAL_FILE)
    ...
    homeplusFile = HOMEPLUS + "_" + Format(DateAdd("d", -1, Date), "MMdd") + "_재고.xlsx"
    
    Dim wbHomePlus As Workbook
    wbHomePlus = Workbooks.Open(FOLDER_PATH_DOWN + stockFile)
    ...
    pWbBIFile.Sheets("자재데이터").Cells(2, 4).PasteSpecial Paste:=xlPasteValues
    ' Clear the clipboard
    Application.CutCopyMode = False
    ...
    pWbBIFile.Sheets("자재데이터").Cells(2, 8).PasteSpecial Paste:=xlPasteValues
    ' Clear the clipboard
    Application.CutCopyMode = False
    'slide 36
    ...
    pWbBIFile.Sheets("자재데이터").Range("I2:I" + CStr(lastVisibleRow)).value = HOMEPLUS
